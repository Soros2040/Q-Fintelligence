...
[特征源码](../../algorithm/qf_algorithm/legacy/quantum_features.py)定义了六量子比特上的 12 个冻结映射。候选在一层/两层、输入缩放 $\{0.5,1,1.5\}$、CZ/受控 RY 链之间变化。每层施加依赖输入的 RY 与 RZ 旋转，第一层先施加 Hadamard 门。量子参数冻结，训练的是经典预测头。
